...
class PaperFetcher:
    ...
    def _parse_pmc_xml(self, xml_text: str) -> str | None:
        ...
        # Require a real full-text <body>. PMC returns only metadata + abstract
        # for articles outside the Open Access subset; treat those as a miss so
        # fetch_full_text falls through to Unpaywall instead of "extracting" from
        # just the abstract.
        body_elem = root.find(".//body")
        if body_elem is None:
            return None
        body_text = self._get_element_text(body_elem)
