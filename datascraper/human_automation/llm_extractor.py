"""
LLM-based mutation data extraction (local Ollama model, or Gemini).
Sends full paper text to the model and gets structured mutation data back.
"""
...
# Created by setup_gemini(); reused across extraction calls.
_client = None

# JSON schema to constrain Ollama output to a valid array of mutation objects
# (prevents prose, markdown, and example-echo from local models).
_OLLAMA_SCHEMA = {
    "type": "array",
    "items": {
        "type": "object",
        "properties": {
            "subunit": {"type": "string", "enum": config.ALL_SUBUNITS},
            "modification_type": {"type": "string", "enum": config.VALID_MODIFICATION_TYPES},
            "aa_position": {"type": "integer"},
            "initial_aa": {"type": "string"},
            "new_aa": {"type": "string"},
            "effect": {"type": "string", "enum": config.VALID_EFFECTS},
            "measuring_technique": {"type": "string"},
            "pathology": {"type": "string"},
        },
        "required": ["subunit", "modification_type", "aa_position",
...
IMPORTANT RULES:
1. ONLY extract mutations of HUMAN (Homo sapiens) nAChR variants. Skip mouse, rat, or
   other species variants unless the paper explicitly states the human gene/protein was used.
   Note: Many papers express human nAChR subunits in Xenopus oocytes or HEK293 cells for
   electrophysiology - these COUNT as human variants (the gene is from human, the expression
...
Return ONLY a JSON array (no prose, no commentary, no markdown fences). Each element must have
exactly these fields:
- "subunit": one of CHRNA1, CHRNA2, CHRNA3, CHRNA4, CHRNA5, CHRNA6, CHRNA7, CHRNA9, CHRNA10,
  CHRNB1, CHRNB2, CHRNB3, CHRNB4, CHRND, CHRNE, CHRNG
- "modification_type": one of Substitution, Frameshift, Stop, Deletion
- "aa_position": integer residue number stated in the paper
- "initial_aa": original amino acid, one-letter code
- "new_aa": new amino acid one-letter code ("*" for Stop, "del" for Deletion, "fs" for Frameshift)
- "effect": one of LOF, GOF, No net effect
- "measuring_technique": the electrophysiology / ion-flux method used
- "pathology": associated disease or condition, or "" if none

Extract ONLY mutations actually described in the PAPER TEXT below. Do NOT invent or copy entries.
If there are NO qualifying human mutations, output exactly: []
...
def setup_gemini():
    """Prepare the configured extraction backend (ollama or gemini)."""
    global _client
    if config.EXTRACT_BACKEND == "ollama":
        try:
            tags = requests.get(f"{config.OLLAMA_URL}/api/tags", timeout=10).json()
        except Exception as e:
            raise RuntimeError(
                f"Cannot reach Ollama at {config.OLLAMA_URL}. Is it installed and running? ({e})"
            )
        names = [m.get("name", "") for m in tags.get("models", [])]
        if not any(config.OLLAMA_MODEL.split(":")[0] in n for n in names):
            raise RuntimeError(
                f"Ollama model '{config.OLLAMA_MODEL}' not found. Run:  ollama pull {config.OLLAMA_MODEL}\n"
                f"Models available: {', '.join(names) or '(none)'}"
            )
        return
...
def _call_backend(prompt: str) -> str:
    """Send the prompt to the configured backend and return its raw text response."""
    if config.EXTRACT_BACKEND == "ollama":
        resp = requests.post(
    ...
    client = _client or genai.Client(api_key=config.GEMINI_API_KEY)
...
def extract_mutations_from_paper(
    paper_text: str, pmid: str, max_text_length: int = 150000
) -> list[dict]:
    """
    ...
    """
    # Truncate long papers to fit the backend's context window.
    limit = config.OLLAMA_MAX_CHARS if config.EXTRACT_BACKEND == "ollama" else max_text_length
    if len(paper_text) > limit:
        paper_text = paper_text[:limit] + "\n\n[TEXT TRUNCATED]"
    ...
    for attempt in range(config.MAX_RETRIES):
        try:
            raw_text = _call_backend(prompt)
            mutations = _parse_gemini_response(raw_text, pmid)
            if config.EXTRACT_BACKEND == "gemini":
                time.sleep(config.GEMINI_DELAY_SECONDS)  # pace under the API rate limit
            return mutations  # success — an empty list here is a genuine "no mutations"

        except Exception as e:
            last_err = e
            msg = str(e).lower()
            is_rate = any(t in msg for t in (
                "429", "resource_exhausted", "quota", "rate limit", "too many requests"))
            wait_time = min(config.RETRY_BACKOFF_FACTOR ** (attempt + 2), 60)
            if is_rate:
                wait_time = max(wait_time, 30)  # rate/quota windows need a longer wait
            print(f"    {config.EXTRACT_BACKEND} {'rate limit' if is_rate else 'error'} on PMID {pmid} "
                  f"(attempt {attempt + 1}/{config.MAX_RETRIES}); waiting {wait_time}s")
            time.sleep(wait_time)

    # All retries exhausted: RAISE so the caller records this paper as a retriable
    # ERROR (re-attempted on the next run) instead of silently writing 0 rows.
    raise RuntimeError(
        f"{config.EXTRACT_BACKEND} extraction failed for PMID {pmid} after {config.MAX_RETRIES} attempts: {last_err}"
    )
