"""
UniProt source: mouse nAChR mutagenesis / variant annotations.

For each mouse subunit gene (Chrna1..Chrng, taxid 10090) UniProt curates
...
def _mouse_gene(subunit: str) -> str:
    # CHRNA7 -> Chrna7  (UniProt gene search is case-insensitive regardless).
    return subunit[0] + subunit[1:].lower()
...
def search_subunit(subunit: str) -> dict:
    params = {
        "query": f"(gene:{_mouse_gene(subunit)}) AND (organism_id:{config.MOUSE_TAXID})",
...
def search_all(subunits: list) -> dict:
    ...
    for subunit in subunits:
        print(f"\nSearching UniProt for {subunit} (mouse)...")
        try:
