...
Searches for nAChR mutation papers with electrophysiology data in human.
"""
...
class PaperMetadata:
    ...
    curated_mutations: list[str] = field(default_factory=list)
    curated_effects: list[str] = field(default_factory=list)
...
def _build_search_query(subunit: str) -> str:
    ...
    species_parts.append(config.ORGANISM_TAG)
...
def _extract_year(art) -> str:
    """Best-effort 4-digit publication year from a PubMed Article record."""
    try:
        pub_date = art["Journal"]["JournalIssue"]["PubDate"]
        if pub_date.get("Year"):
            return str(pub_date["Year"])
        m = re.search(r"\d{4}", str(pub_date.get("MedlineDate", "")))
        if m:
            return m.group(0)
    except (KeyError, TypeError):
        pass
    # Fall back to the electronic article date.
    for adate in art.get("ArticleDate", []):
        if adate.get("Year"):
...
def search_all_subunits(subunits: list[str], checkpoint=None) -> dict[str, PaperMetadata]:
    ...
    # Attach subunit associations and tag the source
