"""
Configuration for the HUMAN nAChR data pipeline (worklist + extraction).
All settings, constants, and API configurations are centralized here.

This is the human counterpart of the mouse pipeline in ../datascraper. It
searches for HUMAN (Homo sapiens) nAChR mutation-electrophysiology papers,
ranks them, and (via extract_main.py) extracts mutation rows to augment the
hand-curated human dataset in nachr_db_manual.xlsx.
"""
...
WORKLIST_EXCEL = OUTPUT_DIR / "nachr_human_worklist.xlsx"
METADATA_CACHE = OUTPUT_DIR / "metadata_cache.json"
CHECKPOINT_FILE = CHECKPOINT_DIR / "checkpoint.json"

# The hand-curated human DB. Worklist "Status" flags which candidate papers are
# already in it (so you can focus on the NEW ones you may have missed).
MANUAL_DB = REPO_DIR / "nachr_db_manual.xlsx"
PRIOR_WORKLIST = None  # no earlier human worklist to cross-reference
...
# MeSH terms for human species (NLM tags human-subject/material papers broadly)
SPECIES_MESH_TERMS = [
    "Humans",
]
...
SPECIES_TEXT_TERMS = [
    ...
    "patient", "patients", "proband", "probands",
    "clinical", "individual", "family", "kindred", "carrier",
]

# Organism tag (PubMed) and free-text organism name (Europe PMC) for the species clause.
ORGANISM_TAG = '"Homo sapiens"[Organism]'
...
UNIPROT_TAXID = 9606                  # Homo sapiens
...
SCORE_WEIGHTS = {
    ...
    "species": 1,             # human / patient / proband / clinical ...
...
# ════════════════════════════════════════════════════════════════════════════
# EXTRACTION PIPELINE  (paper_fetcher.py, llm_extractor.py, excel_writer.py,
# extract_main.py) — fetch full text of the worklist papers and pull mutation
# rows into the nachr_db_manual.xlsx format.
# ════════════════════════════════════════════════════════════════════════════
...
# Gemini (alternative backend) — only used if EXTRACT_BACKEND="gemini"
...
# ── Extraction outputs (this folder) ─────────────────────────────────────────
EXTRACTED_EXCEL = BASE_DIR / "extracted_human_mutations.xlsx"
EXTRACT_INACCESSIBLE = BASE_DIR / "inaccessible_papers.xlsx"
EXTRACT_CHECKPOINT = BASE_DIR / "extract_checkpoint.json"
